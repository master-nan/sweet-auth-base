...
4. 低代码列表、详情、新增、编辑、删除都走统一的数据权限解析；固定系统页列表和详情/编辑/删除检查也复用同一套规则。
...
`sys_data_scope_binding` 声明某个菜单或低代码表受哪些数据权限维度控制，以及该维度落在哪个业务字段上。
...
- `required`：是否必须配置授权范围。
- `actions`：生效动作 JSON，例如 `query`、`detail`、`create`、`update`、`delete`。
- `state`：是否启用。
...
- 角色数据权限：常规权限来源。
- 用户数据权限覆盖：用于临时扩大、收窄、替换或拒绝特殊人员范围。
...
- 低代码详情、编辑、删除和固定系统页记录检查都应走数据权限。
- 文件预览和下载在有业务记录上下文时回到统一数据权限层检查。
