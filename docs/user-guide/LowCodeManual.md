...
| 配置字段输入组件、列表、表单、详情 | 7、17、[字段类型指南](FieldTypeGuide.md) |
| 配置下拉、字典、关联表、级联 | 2、3、4、5、8、[联动配置指南](LinkageConfig.md) |
...
优先在字段管理界面里配置关联表、显示字段、值字段、分页和联动筛选；只有批量初始化、排障或高级配置时才直接编辑 `linkage_config` JSON。完整字段说明见 [联动配置指南](LinkageConfig.md)。
...
| `pageSize` | 每页加载多少条，字段管理默认 200；关联下拉运行时限制在 20-200 |
...
确认弹框和参数弹框的顺序按页面不同略有差异：列表页通常先确认再打开参数表单；详情页通常先填写参数，提交时再确认。无论顺序如何，最终都只向后端提交一次按钮请求。

列表页 `custom` 按钮默认请求体格式：
...
| `closeDialog` | 列表页自定义按钮成功后尝试关闭当前主表单弹框；参数弹框在提交前已经关闭，详情页不提供关闭回调 |
...
低代码表的新增/编辑和详情打开方式由表配置控制：

| 字段 | 可选值 | 说明 |
| --- | --- | --- |
| `form_open_mode` | `auto`、`dialog`、`page` | 控制新增/编辑表单打开方式 |
| `detail_open_mode` | `auto`、`dialog`、`page` | 控制详情打开方式 |

`dialog` 固定弹框，`page` 固定打开页签。`auto` 会按页面复杂度决定：新增/编辑字段较多、有富文本、文件、级联等复杂控件时更倾向打开页签；详情页默认更适合页签，避免内容多时弹框拥挤。
...
5. 树形范围会按维度来源表的父级字段展开；权限排查会展示解析后的范围和诊断备注。

新增没有历史行可查，后端会校验本次提交的受控字段是否落在可访问范围内。编辑会先校验原行是否可访问，再校验本次提交里出现的受控字段；未提交的受控字段按原值处理。当前施工模型见 [通用数据权限设计与实现说明](../_construction/design/DataPermissionDesign.md)。

不要在某个控制器里临时写一套固定业务字段、`table_code`、`field_code` 的判断来当最终数据权限。详情、列表、导出、批量操作、文件预览和下载只要涉及业务记录归属，都应该复用同一套数据权限解析结果。
...
1. 中间件只允许已登录用户进入通用接口。
2. 控制器根据 `tableCode` 找到发布菜单；如果同一张表有多个菜单绑定，建议请求携带 `menu_id`，否则后端可能拒绝无法唯一定位的请求。
...
1. 打开低代码菜单，列表能正常加载。
2. 点击新增，按 `form_open_mode` 打开弹框或页签，字段应按字段元数据渲染。
3. 字典字段应该显示下拉选项。
4. 关联字段应该能搜索和选择目标表数据。
...
9. 点击详情，按 `detail_open_mode` 打开当前行详情弹框或页签。
