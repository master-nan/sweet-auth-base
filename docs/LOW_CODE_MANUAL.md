...
| 配置下拉、字典、关联表、级联 | 2、3、4、5、8、[联动配置指南](linkage_config.md) |
...
旧字段 `is_hidden` 只用于兼容历史数据，不再作为业务配置入口。不要把“接口权限”理解成“隐藏按钮”，它的作用是给后端接口授权，不负责控制页面显示隐藏。
...
不要在某个控制器里临时写一套固定业务字段、`table_code`、`field_code` 的判断来当最终数据权限。详情、列表、导出、批量操作、文件预览和下载都应该复用同一套数据权限解析结果。设计草案见 [通用数据权限设计草案](data-permission-design.md)。
