...
本文依据当前仓库中的 Organization 模型、已冻结的 Integration Runtime/Retry/Sync 设计、本地 OpenAPI，以及 `docs/_construction/analysis/organization-source/` 下的脱敏分析证据编写。本文不授权建立第二条 HTTP、Retry、Scheduler、Checkpoint 或 Execution 链路。
...
正式证据与 Gate 矩阵见 `docs/_construction/reviews/OrganizationHRAssignmentContractReview.md`。本轮没有修改代码、数据库或生产 Consumer，也没有重新采集真实人员数据。
...
正式证据见 `docs/_construction/reviews/OrganizationHRSyncAcceptanceReport.md`，能力冻结见 `docs/_construction/reviews/OrganizationHRSyncFreezeReview.md`。INT-006G 以 INT-006F-B 实现提交为基线，完成代码、Migration、Registry、七类 Consumer、PostgreSQL 16、Integration SyncRunner + WorkerRunner + TLS、Retry/Checkpoint、页面、权限和脱敏验收。
