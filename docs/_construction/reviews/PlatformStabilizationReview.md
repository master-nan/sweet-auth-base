...
- 建设期命名规则见 [DocumentationNamingStandard.md](../standards/DocumentationNamingStandard.md)；当前目录与生命周期规则以 [DocumentationStandard.md](../../DocumentationStandard.md) 为准。
...
- 已建立正式的 [ErrorHandlingStandard.md](../standards/ErrorHandlingStandard.md)，冻结 Repository、Service、Controller/Middleware 三层错误边界，并完成一批明确的错误信息泄露整改。
- 已建立正式的 [TransactionUsageStandard.md](../standards/TransactionUsageStandard.md)，明确 Service 定义业务事务边界、Repository 执行数据访问、Controller 不创建事务。
- 已建立正式的 [TestInfrastructureStandard.md](../standards/TestInfrastructureStandard.md)，统一测试分类、数据库 Helper、Gin 包级初始化、隔离要求及 CI 可见性要求。
...
- 统一认证目标架构已经冻结，密码、短信和未来 SSO 被定义为 Credential Provider，账号状态、安全策略、Token 与审计由统一编排承担，见 [AuthenticationArchitectureDesign.md](../design/AuthenticationArchitectureDesign.md)。存量认证入口按该设计渐进迁移，不在本轮进行一次性重写。
