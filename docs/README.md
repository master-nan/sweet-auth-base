...
| [`_construction/`](_construction/README.md) | 建设与评审人员 | HR 生产 Gate、Integration Sync 前置证据和 Report 重设计参考 | 临时；不是产品文档 |
...
Organization HR、其 Integration Sync 前置证据和 Report 重设计参考位于 `_construction/`。Organization HR 的真实生产源 Gate 尚未关闭，相关资料不得提前删除，也不得据此启用生产 Consumer。

## 清理原则

- 通用 Acceptance、Freeze、Audit、阶段 Review、旧 Standard 和实现 Evidence 已在 RC-001 完成第一轮删除，Git 历史负责追溯。
- 当前 construction 只保留 HR/Integration Sync 生产准入证据和 Report 重设计参考，不机械清理。
