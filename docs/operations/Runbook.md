...
低代码发布、字段配置、按钮动作、参数 Schema、关联下拉、权限排查和常用模板见 [LowCodeManual.md](../user-guide/LowCodeManual.md)。
...
默认本地配置连接 PostgreSQL `127.0.0.1:5432`、Redis `127.0.0.1:6379`。如果想让本地 `go run main.go` 复用 `make docker-up` 启动的容器依赖，需要覆盖为 PostgreSQL `15432`、Redis `16380`，或把 [backend/config-dev.yaml](../../backend/config-dev.yaml) 调整成你的本机端口。
...
```bash
make db-migrate-external
make db-seed-external
```

不要用 `make db-migrate` / `make db-seed` 操作外部库；这两个命令读取本地 [backend/config-dev.yaml](../../backend/config-dev.yaml)。外部库命令会读取 `.env.external` 并通过 `docker-compose.external.yml` 临时运行迁移程序。
...
连接已有 PostgreSQL/Redis 时使用外部环境命令：
...
| 外部库迁移 | `make db-migrate-external` |
| 外部库初始化内置数据 | `make db-seed-external` |
