...
func TestProvideIntegrationWorkerRunnerConfigDefaultsDisabled(t *testing.T) {
	...
	if value.Enabled || value.WorkerID != "integration-disabled" || value.PollInterval != 5*time.Second || value.ClaimBatchSize != 8 || value.InstanceConcurrency != 4 {
...
func TestProvideIntegrationWorkerRunnerConfigUsesConfiguredWorkerIdentity(t *testing.T) {
	value, err := ProvideIntegrationWorkerRunnerConfig(&config.Server{Integration: config.Integration{Worker: config.IntegrationWorker{
		Enabled: true, WorkerID: "runtime-instance-7", PollInterval: 6, ClaimBatchSize: 2, InstanceConcurrency: 2, LeaseRecoveryInterval: 20, ShutdownTimeout: 8,
	}}})
	if err != nil {
		t.Fatalf("provide configured worker: %v", err)
	}
	if !value.Enabled || value.WorkerID != "runtime-instance-7" || value.PollInterval != 6*time.Second {
