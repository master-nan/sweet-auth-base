...
func InitializeApp() (*App, error) {
	...
	metadataRuntimeService := service.NewMetadataRuntimeService(sysTableRepositoryImpl, sysTableFieldRepositoryImpl, sysTableCache, sysTableFieldCache)
	sysTableService := service.NewSysTableService(sysTableRepositoryImpl, sysTableFieldRepositoryImpl, sysTableIndexRepositoryImpl, sysTableIndexFieldRepositoryImpl, sysTableRelationRepositoryImpl, snowflake, metadataRuntimeService)
	...
	authAuditService := service.NewAuthAuditService(logService)
	captchaVerifier := service.NewCaptchaVerifier()
	passwordCredentialProvider := service.NewPasswordCredentialProvider(sysUserRepositoryImpl, server)
	sendCodeCache := cache.NewSendCodeCache(redisUtil)
	smsCredentialProvider := service.NewSMSCredentialProvider(sysUserRepositoryImpl, sendCodeCache)
	...
	lowCodePublicationService := service.NewLowCodePublicationService(sysTableRepositoryImpl, sysMenuRepositoryImpl, sysMenuButtonRepositoryImpl, sysMenuButtonTemplateRepositoryImpl, sysRoleRepositoryImpl, sysRoleMenuRepositoryImpl, sysRoleMenuButtonRepositoryImpl, snowflake, metadataRuntimeService)
	...
	sysMenuService := service.NewSysMenuService(sysMenuRepositoryImpl, sysRoleMenuRepositoryImpl, sysRoleRepositoryImpl, sysRoleMenuButtonRepositoryImpl, sysUserRoleRepositoryImpl, sysMenuButtonRepositoryImpl, metadataRuntimeService, casbinRuleRepositoryImpl, snowflake)
	tableController := controller.NewTableController(sysTableService, lowCodePublicationService, metadataRuntimeService, sysMenuService, v2)
	...
	dataOwnershipConfigService := service.NewDataOwnershipConfigService(dataResourceRepositoryImpl, dataDimensionDefinitionRepositoryImpl, dataOwnershipFieldRepositoryImpl, metadataRuntimeService, ownershipFieldRegistry, snowflake, logService)
	...
	metadataFieldAdapter, err := datapermission.NewMetadataFieldAdapter(metadataRuntimeService)
	if err != nil {
		return nil, err
	}
	...
	generalizationService := service.NewGeneralizationServiceWithRuntimeAndDataPermission(generalizationRepositoryImpl, snowflake, metadataRuntimeService, lowCodeDataPermissionRuntime)
	...
	querySchemeRepositoryImpl := impl.NewQuerySchemeRepositoryImpl(primaryDB)
	registry := queryscheme.NewRegistry()
	querySchemeService := service.NewQuerySchemeService(querySchemeRepositoryImpl, registry, metadataRuntimeService, snowflake, logService)
	querySchemeController := controller.NewQuerySchemeController(querySchemeService, v2)
	...
	generalizationController := controller.NewGeneralizationController(generalizationService, sysMenuService, v2)
	...
	fileBusinessAccessAdapter := controller.NewFileBusinessAccessAdapter(fileAccessService, generalizationService)
