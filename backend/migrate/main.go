...
func seedDataPermissionMenuButtons(db *gorm.DB, sf *utils.Snowflake, roleID int, roleName string, menuID int) error {
	buttons := []model.SysMenuButton{
		...
		apiPermissionWithAPI(605, menuID, "维度选项", "system_data_permission_dimension_options", enum.Line, "query", "list", "primary", 92, "/admin/data-permission/dimension/:code/options", "GET"),
...
func seedSuperAdminRoutePolicies(db *gorm.DB, roleName string) error {
	policies := []routePolicy{
		...
		{"/admin/data-permission/dimension/:code/options", "GET"},
