...
func seedSystemMenuButtons(db *gorm.DB, sf *utils.Snowflake, roleID int, roleName string, menuID int) error {
	...
	if err := seedMenuButtons(db, sf, roleID, roleName, buttons); err != nil {
		return err
	}
	return removeMenuRefreshCacheArtifacts(db, menuID)
}

func removeMenuRefreshCacheArtifacts(db *gorm.DB, menuID int) error {
	return db.Transaction(func(tx *gorm.DB) error {
		var buttonIDs []int
		if err := tx.Unscoped().Model(&model.SysMenuButton{}).
			Where("menu_id = ? AND code = ?", menuID, "system_menu_refresh_cache").
		...
		return tx.Where("ptype = ? AND v1 = ? AND v2 = ?", "p", "/admin/menu/refresh-cache", "POST").
