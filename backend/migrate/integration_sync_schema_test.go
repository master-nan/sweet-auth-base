...
func TestIntegrationSyncSchemaPostgreSQLConstraints(t *testing.T) {
	...
	db, err := testutil.OpenPostgres(t, postgres.Open(postgresDSNWithSearchPath(t, dsn, schemaName)), &gorm.Config{NamingStrategy: schema.NamingStrategy{SingularTable: true}, DisableForeignKeyConstraintWhenMigrating: true, Logger: logger.Default.LogMode(logger.Silent), NowFunc: model.Now})
	if err != nil {
		t.Fatal(err)
	}
	if err := migrateIntegrationConfigurationSchema(db); err != nil {
