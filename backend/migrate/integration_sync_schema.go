...
func migrateIntegrationSyncSchema(db *gorm.DB) error {
	return db.Transaction(func(tx *gorm.DB) error {
		...
		checks := []postgresCheckConstraint{
			...
			{model: &model.IntegrationSyncBatch{}, name: "chk_integration_sync_batch_identity", expression: "btrim(batch_no) <> '' AND btrim(trigger_key) <> '' AND task_version > 0 AND interface_version > 0 AND consumer_version > 0"},
