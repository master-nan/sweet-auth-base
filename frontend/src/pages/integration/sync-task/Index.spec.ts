...
const permissions = vi.hoisted(() => ({ values: ['integration_sync_task_metadata', 'integration_external_system_query', 'integration_interface_definition_query', 'integration_sync_task_consumer_metadata'] }))
...
const row: SyncTaskListItem = { id: 1, task_code: 'employee_sync', task_name: '人员同步', version: 1, status: 'draft', external_system: { id: 1, code: 'hr', name: 'HR' }, interface_definition: { id: 2, code: 'employee', name: '人员', version: 1 }, consumer: { code: 'org_employee', name: '', version: 1 }, schedule_type: 'none', timezone: 'UTC', checkpoint_mode: 'timestamp', lookback_seconds: 0, window_slice_seconds: 3600, input_plan_summary: { version: 1, static_parameter_count: 0, has_window_bindings: true }, revision: 1, gmt_modify: '' }
describe('sync task page permissions', () => {
  beforeEach(() => { setActivePinia(createPinia()); permissions.values = ['integration_sync_task_metadata', 'integration_external_system_query', 'integration_interface_definition_query', 'integration_sync_task_consumer_metadata']; Object.values(api).forEach((mock) => mock.mockReset()); notify.mockReset(); tableApi.queryTableByCode.mockReset(); api.querySyncTasks.mockResolvedValue({ data: [row], total: 1 }); api.queryExternalSystems.mockResolvedValue({ data: [] }); api.queryInterfaceDefinitions.mockResolvedValue({ data: [] }); api.listSyncConsumers.mockResolvedValue({ data: [] }); tableApi.queryTableByCode.mockResolvedValue({ data: { table_fields: [] } }) })
  ...
  it('does not request protected form metadata for a query-only account', async () => {
    permissions.values = []
    ...
    expect(tableApi.queryTableByCode).not.toHaveBeenCalled()
