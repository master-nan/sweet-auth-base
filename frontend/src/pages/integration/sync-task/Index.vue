    ...
    <q-table v-model:pagination="pagination" class="fit sticky-header-table" color="primary" :dense="$q.screen.lt.md" separator="cell" flat bordered :rows="rows" :columns="columns" :visible-columns="visibleColumns" row-key="id" :loading="loading">
      <template #top>
        <standard-table-toolbar :refreshing="loading" @refresh="fetchData">
          ...
          <q-input v-model="keyword" dense outlined debounce="300" placeholder="搜索任务编码或名称" @keyup.enter="handleBasicSearch"><template #append><q-icon name="search" /></template></q-input>
...
const $q = useQuasar(); const api = useIntegrationApi(); const loading = ref(false); const loadError = ref(''); const { confirmAction } = useConfirmDialog($q)
const { line_buttons, top_buttons, has_line_buttons, hasGrantedCapability } = usePageButtons('integration_sync_task')
...
const systems = ref<ExternalSystemListItem[]>([]); const interfaces = ref<InterfaceDefinitionListItem[]>([]); const consumers = ref<SyncConsumerMetadata[]>([])
const { fields: metadataFields, advancedSearchFields: advancedFields, loadMetadata } = useRuntimeTableMetadata('integration_sync_task')
...
const columns = ref<TableColumn<SyncTaskListItem>[]>([]); const visibleColumns = ref<string[]>([]); const sortableFields = ref<ReadonlySet<string>>(new Set()); const emptyExpressions = () => [{ rules: [{ field: '', value: null }], nested: [] }]
const queryState = useTableQueryState<SyncTaskQuery>({ createInitialQuery: () => ({ page: 1, num: 15, order: { field: '', is_asc: false }, quick_query: { keyword: '' }, expressions: emptyExpressions() }), createEmptyExpressions: emptyExpressions }); const { query, keyword, draftAdvanced: tempAdvancedQuery } = queryState; const pagination = ref({ page: 1, rowsPerPage: 0, sortBy: '', descending: true })
const emptyMessage = computed(() => resolveTableEmptyMessage({ canRead: canQueryTasks.value, error: loadError.value, hasQuery: !!keyword.value }))
const fetchData = async () => { if (!canQueryTasks.value) return; loading.value = true; loadError.value = ''; try { const result = await api.querySyncTasks(query.value); rows.value = result.data || []; total.value = result.total || 0 } catch { rows.value = []; total.value = 0; loadError.value = '同步任务加载失败' } finally { loading.value = false } }
const fetchMetadata = async () => { if (!(await loadMetadata())) return; const resolution = resolveRuntimeColumns<SyncTaskListItem>(metadataFields.value, { context: { getDictLabel: () => '' }, overrides: [{ fieldCode: 'task_name', visible: false }, { fieldCode: 'interface_definition_id', visible: false }, { fieldCode: 'consumer_code', visible: false }, { fieldCode: 'consumer_version', visible: false }, { fieldCode: 'schedule_type', visible: false }, { fieldCode: 'checkpoint_at', visible: false }, { fieldCode: 'task_code', label: '同步任务', order: 1 }, { fieldCode: 'status', align: 'center', order: 2 }, { fieldCode: 'window_slice_seconds', label: '切片（秒）', align: 'right', order: 7 }], virtualColumns: [{ name: 'interface', label: '接口版本', field: 'interface_definition', order: 3 }, { name: 'consumer', label: 'Consumer', field: 'consumer', order: 4 }, { name: 'schedule', label: '调度', field: 'schedule_type', order: 5, serverSortField: 'schedule_type' }, { name: 'checkpoint', label: 'Checkpoint', field: 'checkpoint_at', order: 6, serverSortField: 'checkpoint_at' }, { name: 'actions', label: '操作', field: 'actions', align: 'center', order: 100, defaultVisible: has_line_buttons.value }] }); columns.value = resolution.columns; visibleColumns.value = resolution.visibleColumns; sortableFields.value = resolution.sortableFields }
const fetchReferences = async () => {
  const requests: Promise<void>[] = []
  if (hasGrantedCapability('integration_external_system_query')) requests.push(api.queryExternalSystems({ page: 1, num: 500, order: { field: 'name', is_asc: true }, expressions: emptyExpressions(), status: 'enabled' }).then((result) => { systems.value = result.data || [] }))
  if (hasGrantedCapability('integration_interface_definition_query')) requests.push(api.queryInterfaceDefinitions({ page: 1, num: 500, order: { field: 'interface_code', is_asc: true }, expressions: emptyExpressions(), status: 'enabled' }).then((result) => { interfaces.value = result.data || [] }))
  if (hasGrantedCapability('integration_sync_task_consumer_metadata')) requests.push(api.listSyncConsumers().then((result) => { consumers.value = result.data || [] }))
...
const handlers: PageActionHandlers<SyncTaskListItem> = { create: () => { void openCreate() }, detail: (row) => { if (row) { detailID.value = row.id; showDetail.value = true } }, update: (row) => row && void openEdit(row), create_version: (row) => row && createVersion(row), enable: (row) => row && changeState(row, true), disable: (row) => row && changeState(row, false), run: (row) => row && runTask(row) }
const handleButtonClick = (button: MenuButton, row?: SyncTaskListItem) => { dispatchPageAction(button, handlers, row) }
