...
const apiMocks = vi.hoisted(() => ({
  queryCredentials: vi.fn(), queryExternalSystems: vi.fn(), getCredential: vi.fn(),
...
const buttons = vi.hoisted(() => ({
  ...
  line: [
    ...
    { id: 4, name: '轮换', event_action: 'rotate' }, { id: 5, name: '启用', event_action: 'enable' },
    { id: 6, name: '停用', event_action: 'disable' }, { id: 7, name: '吊销', event_action: 'revoke' },
  ],
}))
...
const ButtonStub = defineComponent({ name: 'QBtn', props: { label: { type: String, default: '' } }, emits: ['click'], setup(props, { emit }) { return () => h('button', { onClick: () => emit('click') }, props.label) } })
const mountPage = () => shallowMount(CredentialPage, { global: { plugins: [createPinia()], stubs: {
  ...
  CredentialFormDialog: true, CredentialDetailDialog: true,
} } })
...
const row = { id: 41, external_system: { id: 10, system_code: 'demo_erp', name: 'Demo ERP' }, credential_code: 'erp_api', name: 'ERP API Key', credential_type: 'api_key', status: 'draft', effective_status: 'draft', fingerprint_summary: 'abcdef123456', version: 1, revision: 1, gmt_modify: '' }
...
describe('credential management page', () => {
  ...
  it('hides every mutating action after revocation', async () => {
    ...
    expect(vm.availableLineButtons(row).map((item) => item.event_action)).toEqual(['detail', 'update', 'rotate', 'enable', 'revoke'])
    expect(vm.availableLineButtons({ ...row, status: 'revoked', effective_status: 'revoked' }).map((item) => item.event_action)).toEqual(['detail'])
  })
})
