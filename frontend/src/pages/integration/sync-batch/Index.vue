  ...
  <base-content class="q-pa-sm">
    ...
    >
      ...
      <template #body-cell-batch_no="props">
        <q-td :props="props">
          <q-btn v-if="canDetail" flat dense no-caps color="primary" :label="props.row.batch_no" @click="openDetail(props.row.id)" />
          <span v-else class="text-mono">{{ props.row.batch_no }}</span>
          <div class="text-caption text-grey-7">{{ props.row.task_name }} · v{{ props.row.task_version }}</div>
        </q-td>
      </template>
      ...
      <template #body-cell-reason="props"><q-td :props="props">{{ props.row.reason_code || '-' }}</q-td></template>
      ...
      <template v-if="detail">
        ...
        <q-list v-else bordered separator>
          <q-item v-for="execution in executions" :key="execution.id">
            <q-item-section>
              <q-item-label>
                <q-btn v-if="canViewExecutionDetail" flat dense no-caps color="primary" :label="execution.execution_no" @click="openExecution(execution.id)" />
                <span v-else class="text-mono">{{ execution.execution_no }}</span>
              </q-item-label>
              <q-item-label caption>
                Slice {{ execution.sync_source?.slice_no || '-' }} · {{ formatDate(execution.sync_source?.window_start) }} 至 {{ formatDate(execution.sync_source?.window_end) }}
              </q-item-label>
            </q-item-section>
            <q-item-section side>{{ execution.status }}</q-item-section>
          </q-item>
          <q-item v-if="executions.length === 0"><q-item-section class="text-grey-7">暂无 Execution</q-item-section></q-item>
        </q-list>
      </template>
    </form-dialog-shell>
  </base-content>
...
const triggerOptions = [{ label: '手工', value: 'manual' }, { label: '定时', value: 'scheduled' }]
const triggerLabel = (value: string) => triggerOptions.find((item) => item.value === value)?.label || value
const formatDate = formatRuntimeDateTime
const columns: QTableProps['columns'] = [
  { name: 'batch_no', label: '批次', field: 'batch_no', align: 'left', sortable: true },
  { name: 'trigger_type', label: '触发类型', field: 'trigger_type', align: 'center', sortable: true },
...
const openDetail = async (id: number) => {
  ...
  executions.value = []
  try {
    const requests: Promise<unknown>[] = [api.getSyncBatch(id).then((result) => { detail.value = result.data || null })]
    if (canQueryExecutions.value) {
      requests.push(api.queryExecutions({ page: 1, num: 100, order: { field: 'sync_slice_no', is_asc: true }, expressions: [{ rules: [{ field: '', value: null }], nested: [] }], sync_batch_id: id }).then((result) => { executions.value = result.data || [] }))
    }
...
const openExecution = (id: number) => { void router.push({ name: 'integration_execution_detail_page', params: { id } }) }
const detailItems = computed(() => detail.value ? [
  ...
  { label: '接口版本', value: `${detail.value.interface_code} · v${detail.value.interface_version}` },
  { label: 'Consumer', value: `${detail.value.consumer_code} · v${detail.value.consumer_version}` },
  { label: '逻辑窗口', value: `${formatDate(detail.value.window_start)} 至 ${formatDate(detail.value.window_end)}` },
  { label: 'Checkpoint', value: `${formatDate(detail.value.checkpoint_before)} 至 ${formatDate(detail.value.checkpoint_after)}` },
  { label: '切片进度', value: `${detail.value.current_slice_no} / ${detail.value.planned_slice_count}（Execution ${detail.value.execution_count}）` },
  { label: '技术结果', value: `成功 ${detail.value.technical_success_count} / 失败 ${detail.value.technical_failed_count}` },
  { label: '业务结果', value: `成功 ${detail.value.business_success_count} / 失败 ${detail.value.business_failed_count}` },
  { label: '原因', value: detail.value.reason_code || '-' },
  { label: '开始 / 完成', value: `${formatDate(detail.value.started_at)} / ${formatDate(detail.value.completed_at)}` },
