...
const permissions = vi.hoisted(() => ({ values: ['integration_sync_batch_query', 'integration_sync_batch_detail'] }))
...
describe('sync batch query page', () => {
  beforeEach(() => { permissions.values = ['integration_sync_batch_query', 'integration_sync_batch_detail']; Object.values(api).forEach((mock) => mock.mockReset()) })
  it('does not preload batch data without query permission', async () => {
    permissions.values = ['integration_sync_batch_detail']
    shallowMount(Page, { global: { stubs: { BaseContent: Slot, QTable: Table, QInput: true, QSelect: true, QBtn: true, QIcon: true, QChip: true, QTd: Slot, QSpace: true, TablePagination: true, FormDialogShell: true } } })
    await flushPromises()
  ...
  it('loads batch executions through the protected execution query', async () => {
    permissions.values = ['integration_sync_batch_query', 'integration_sync_batch_detail', 'integration_execution_query', 'integration_execution_detail']
    api.querySyncBatches.mockResolvedValue({ data: [], total: 0 })
