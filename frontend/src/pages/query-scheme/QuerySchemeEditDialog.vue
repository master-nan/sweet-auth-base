<template>
  <q-dialog v-model="visible" :maximized="$q.screen.lt.md" persistent>
    <q-card style="width: 900px; max-width: 100%">
      <q-card-section class="row items-center">
        <div class="text-h6">{{ detail ? t('ui.editQueryScheme') : t('ui.newQueryScheme') }}</div>
        <q-space />
        <q-btn
          v-close-popup
          flat
          ...
          dense
          icon="close"
          :aria-label="t('ui.closeTheProgramEditWindow')"
          ><q-tooltip>{{ t('ui.close') }}</q-tooltip></q-btn
        >
      </q-card-section>
      <q-separator />
      <q-card-section class="q-gutter-md">
        ...
        <div class="row items-center q-gutter-lg">
          <q-checkbox
            v-if="
              schemeType === QuerySchemeType.PERSONAL || schemeType === QuerySchemeType.PAGE_DEFAULT
            "
            v-model="isDefault"
            :label="
              schemeType === QuerySchemeType.PERSONAL
                ? t('ui.setAsMyDefaultScheme')
                : t('ui.setAsPageDefaultScheme')
            "
          />
        </div>
        <role-select
          ...
          v-model="roleIds"
          :label="t('ui.applicableRolesUpTo32')"
          :rules="[validateRoleIds]"
        />
        <q-banner v-if="scopeError" class="bg-warning text-dark rounded-borders">{{
          scopeError
        }}</q-banner>
        <q-btn
          outline
          color="primary"
          icon="tune"
          :label="t('ui.editQueryConditions')"
          :disable="!scopeConfig"
  ...
  <advanced-query
    ...
    :title="t('ui.editQueryConditions')"
...
const query = ref<Query>({
  ...
  quick_query: { keyword: '' },
  expressions: [{ logic: ExpressionLogic.AND, rules: [{ field: '', value: null }], nested: [] }],
})
...
const valid = computed(
  () =>
    !!scopeCode.value &&
    !!name.value.trim() &&
    (props.schemeType !== QuerySchemeType.ROLE ||
      (roleIds.value.length > 0 && roleIds.value.length <= 32)),
)
const validateRoleIds = (value: number[]) =>
  (value.length > 0 && value.length <= 32) || t('ui.select1To32Roles')
...
const loadScope = async () => {
  ...
  try {
    ...
    if (!scopeConfig.value) throw new Error(t('ui.queryRangeNotAvailable'))
    fields.value =
      (await tableApi.queryRuntimeTableByCode(scopeConfig.value.table_code)).data?.table_fields ||
      []
  } catch (cause) {
    scopeError.value = cause instanceof Error ? cause.message : t('ui.loadingOfQueryRangesFailed')
  }
}
...
const reset = async () => {
  ...
  query.value = {
    page: 1,
    num: 20,
    order: source?.order || { field: '', is_asc: false },
    quick_query: source?.quick_query || { keyword: '' },
    expressions: source?.expressions || [
      { logic: ExpressionLogic.AND, rules: [{ field: '', value: null }], nested: [] },
    ],
  }
...
const submit = async () => {
  ...
  try {
    if (props.schemeType === QuerySchemeType.PERSONAL) {
      if (!props.detail) throw new Error(t('ui.newPersonalSchemeFromTheBusinessPage'))
      await api.updatePersonal(props.detail.id, {
        name: name.value.trim(),
        query_payload: payload.value,
        is_default: isDefault.value,
        revision: props.detail.revision,
      })
    } else if (props.detail) {
      await api.updateShared(props.detail.id, {
        name: name.value.trim(),
        query_payload: payload.value,
        is_default: props.schemeType === QuerySchemeType.PAGE_DEFAULT ? isDefault.value : false,
        role_ids: props.schemeType === QuerySchemeType.ROLE ? roleIds.value : [],
        revision: props.detail.revision,
      })
    } else {
      await api.createShared({
        name: name.value.trim(),
        scope_code: scopeCode.value,
        scheme_type: props.schemeType as Exclude<SchemeType, 'PERSONAL'>,
        query_payload: payload.value,
        is_default: props.schemeType === QuerySchemeType.PAGE_DEFAULT ? isDefault.value : false,
        enabled: true,
        role_ids: props.schemeType === QuerySchemeType.ROLE ? roleIds.value : [],
