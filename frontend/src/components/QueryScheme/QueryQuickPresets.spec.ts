...
describe('QueryQuickPresets', () => {
  it('only exposes date presets with an explicit quick date field and emits bindings', async () => {
    const wrapper = mount(QueryQuickPresets, { props: { config: { scope_code: 'demo', scope_label: '示例', table_code: 'demo', quick_date_field: 'created_at', quick_presets: [], virtual_sort_fields: [], dynamic_binding_kinds: Object.values(QuerySchemeBindingKind) } }, global: { stubs: { QBtn: ButtonStub } } })
  ...
  it('does not guess a date field', () => {
    const wrapper = mount(QueryQuickPresets, { props: { config: { scope_code: 'demo', scope_label: '示例', table_code: 'demo', quick_presets: [], virtual_sort_fields: [], dynamic_binding_kinds: [] } }, global: { stubs: { QBtn: ButtonStub } } })
