...
const scrollToPosition = (value: number, duration = 0) => {
...
const onScroll = (info: scrollInfo) => {
  if (info.verticalPercentage >= 0.1) {
...
onMounted(() => {
  ...
  const t: scrollPosition | null = LocalStorage.getItem(basePath.value)
  if (t) {
...
onUnmounted(() => {
  LocalStorage.remove(basePath.value)
})

onActivated(() => {
  const t: scrollPosition | null = LocalStorage.getItem(route.fullPath)
  if (t) {
    scrollToPosition(t.top)
  }
})
...
onDeactivated(() => {
  ...
  LocalStorage.set(basePath.value, position)
})
</script>
